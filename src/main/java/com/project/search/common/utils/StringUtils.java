...
//        return sdf.format(DateUtils.getNow()).substring(2, 8) + String.format("%010d", hashCode);
    ...
    public static  List<String> substringString(String string) {
        List<String> result = new ArrayList<>();
        if (StringUtils.isEmpty(string)) {
            return result;
        }
        int index = 0;
        int currIndex = string.indexOf(",", index);
        if (currIndex > -1) {
            while (currIndex > -1 || index < string.length()) {
                String str = "";
                if (currIndex < 0) {
                    str = string.substring(index, string.length());
                    currIndex = index + str.length();
                } else {
                    str = string.substring(index, currIndex);
                }
                if (StringUtils.isNoneBlank(str)) {
                    result.add(str);
                }
                index = ++currIndex;
                currIndex = string.indexOf(",", index);
            }
        } else {
            result.add(string);
        }
